...
fn spawn_player(mut commands: Commands, asset_server: Res<AssetServer>, mut atlas_layout: ResMut<Assets<TextureAtlasLayout>>) {
    let texture: Handle<Image> = asset_server.load("character-spritesheet.png");
...
fn move_player(input: Res<ButtonInput<KeyCode>>, time: Res<Time>, mut player: Query<(&mut Transform, &mut AnimationState), With<Player>>) {
    ...
    if direction != Vec2::ZERO {
        ...
        anim.moving = true;
        if direction.x.abs() > direction.y.abs() { // I really dont like this statement but i dont wanna figure out something better right now
            anim.facing = if direction.x > 0.0 { Facing::Right } else { Facing::Left }; // maybe later who knows
        } 
        else {
...
fn animate_player(time: Res<Time>, mut query: Query<(&mut AnimationState, &mut AnimationTimer, &mut Sprite), With<Player>>) {
    ...
    let just_started: bool = anim.moving && !anim.was_moving;
    let just_stopped: bool = !anim.moving && anim.was_moving;
    ...
    if anim.moving {
        if just_started {
            ...
            timer.tick(time.delta()); //timer is lazy, apparently; advancing it manually
